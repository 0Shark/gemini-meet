...
from gemini_meet_client.data_types import (
    MeetingChatHistory,
...
from gemini_meet_client.utils import is_async_context, name_in_transcript
...
class GeminiMeetClient:
    """Client for interacting with the gemini_meet server."""
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the GeminiMeetClient with the server URL.

        Args:
            url (str | FastMCP): The URL of the Gemini Meet server or a
                FastMCP instance.
            name (str | None): The name of the participant, defaults to "gemini_meet".
        ...
        self.name: str = name or self.settings.get("name", "gemini")
    ...
    async def __aenter__(self) -> Self:
        """Connect to the gemini_meet server."""
        await self._connect()
        return self

    async def __aexit__(self, *_exc: object) -> None:
        """Disconnect from the gemini_meet server."""
    ...
    async def _connect(self) -> None:  # noqa: C901
        ...
        if self._client is not None:
            msg = "Already connected to the gemini_meet server"
            raise RuntimeError(msg)
        ...
        if isinstance(self.url, str):
            transport = StreamableHttpTransport(
                url=self.url,
                headers={"gemini-meet-settings": json.dumps(self.settings)},
            )
            logger.info("Connecting to gemini_meet server at %s", self.url)
        else:
            transport = self.url
        ...
        try:
            ...
            logger.exception("Failed to connect to gemini_meet server")
            await self._stack.aclose()
            raise
        else:
            logger.debug("Connected to gemini_meet server")
    ...
    async def list_tools(self) -> list[Tool]:
        """List the available tools on the gemini_meet server.
    ...
    async def join_meeting(
        ...
        """Join a meeting on the gemini_meet server.
    ...
    async def speak_text(self, text: str) -> None:
        """Speak the given text using the gemini_meet server.
