...
from gemini_meet.settings import Settings, get_settings, reset_settings, set_settings
...
from gemini_meet.utils.datadog import create_span, set_span_tag
from gemini_meet.utils.usage import get_usage, reset_usage, set_usage
...
def _extract_settings() -> Settings:
    ...
    try:
        ...
        header = (
            request.headers.get("gemini-meet-settings") if request is not None else None
        )
    except RuntimeError:
    ...
    try:
        ...
        msg = "Invalid gemini_meet-settings."
...
mcp = FastMCP("gemini-meet", lifespan=session_lifespan)
...
async def send_chat_message(
    ...
    # Filter out internal artifact IDs
    if "artifact_pyd_ai_" in message:
        import re

        # Replace the artifact ID with a friendly message
        message = re.sub(
            r"artifact_pyd_ai_[a-f0-9]+", "[Snapshot saved locally]", message
        )
